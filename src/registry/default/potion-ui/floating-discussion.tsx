...
import {
  ...
  useFakeUserInfo,
} from '@/registry/default/potion-ui/block-discussion';
...
const useCommentingNode = () => {
  return useEditorSelector((editor) => {
    ...
    return editor.api.node<TCommentText>({
      match: (n) =>
        TextApi.isText(n) && n[commentsPlugin.key] && n[getDraftCommentKey()],
    })?.[0];
  }, []);
};
...
export const FloatingDiscussion = () => {
  ...
  const commentApi = editor.getApi(commentsPlugin);
  ...
  const updateTimestamp = usePluginOption(commentsPlugin, 'updateTimestamp');
...
const FloatingCommentsContent = ({
  ...
  const setHoverId = (id: string | null) => {
    ...
    editor.setOption(commentsPlugin, 'hoverId', id);
  };

  const highlightDiscussion = (editor: PlateEditor, id: string) => {
    editor.setOption(commentsPlugin, 'activeId', id);
    editor.setOption(suggestionPlugin, 'activeId', null);
    const leaf = editor.api.node({
      at: [],
      match: (n) =>
        TextApi.isText(n) &&
        n[commentsPlugin.key] &&
        editor.getApi(commentsPlugin).comment.nodeId(n) === id,
    });
    ...
    editor
      ...
      .blockSelection.addSelectedRow(parent!.id, {
        clear: false,
        delay: 1000,
      });
  };
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'absolute right-20 z-10 w-72 animate-fade-in cursor-pointer rounded-lg border bg-black p-3 transition-transform duration-200',
...
const FloatingSuggestionContent = ({
  ...
  const highlightSuggestion = (editor: PlateEditor, id: string) => {
    ...
    editor.setOption(commentsPlugin, 'activeId', null);
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "absolute right-20 z-10 w-72 animate-fade-in cursor-pointer rounded-lg border p-3 transition-transform duration-200 text-white",
        "[&[data-hover=true][data-active=false]]:-translate-x-2 [&[data-hover=true][data-active=false]]:bg-black",
        "[&[data-active=true]]:-translate-x-2"
      )}
      style={{
        top,
        background: "linear-gradient(125deg,rgba(152, 107, 174),rgb(50, 81, 96))",
        borderWidth: "0.2px",
        borderColor: "rgb(0, 119, 167)",
    ...
    >
      <div className="flex flex-col">
        <div className="relative flex items-center">
          ...
          <h4 className="text-sm leading-none font-semibold text-black">
            {userData?.name}
          </h4>
