...
import { ConversionDictionary } from './turn-into-dropdown-menu';
...
const groups: Group[] = [
  ...
  // },
  {
    ...
    items: [
      ...
      {
        description: 'Create a table for displaying data.',
        icon: <TableIcon />,
        label: 'Table',
        value: TablePlugin.key,
      },
      {
        description: 'Create a spreadsheet for editing data.',
        icon: <TableIcon />,
        label: 'Spreadsheet',
        value: SpreadsheetPlugin.key,
      },
      {
  ...
  {
    group: 'Turn into',
    items: ConversionDictionary.default.map((item) => ({
      ...item,
      onSelect: (editor) => {
              ...
              {items.map(
                ({
                  ...
                  >
                    {description ? (
                      <>
                        <div className="flex size-11 items-center justify-center rounded border border-foreground/15 bg-black [&_svg]:size-5 [&_svg]:text-subtle-foreground">
                          {icon}
                        </div>
